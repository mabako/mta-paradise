				...
				if username and password then
					...
					if #username >= 3 and #password >= 8 then
						...
						elseif info.usercount == 0 then
							...
							-- create the user
							if exports.sql:query_free( "INSERT INTO wcf1_user (username,password,salt) VALUES ('%s', SHA1(CONCAT('%s', SHA1(CONCAT('%s', SHA1('%s'))))), '%s')", username, salt, salt, password, salt ) then
